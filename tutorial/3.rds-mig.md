...
Here's some information about disabling AWS CLI output:
To disable an output in AWS CLI, use the update-flow-output command with the --output-status DISABLED option. 
To disable automatic pagination in AWS CLI, use the --no-paginate option. This will disable pagination tokens on the client side. 
To specify the formatting style for command output, use the --output parameter. Valid options include json, text, and table. 
Command line options override settings in other locations, such as environment variables.

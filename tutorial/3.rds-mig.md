...
aws rds describe-db-clusters | jq '.DBClusters[].DBClusterMembers[] | select(.IsClusterWriter == true).DBInstanceIdentifier'


aws rds describe-db-clusters | jq '.DBClusters[] | select(.DBClusterIdentifier == "grav-aurora-cluster")'
```
